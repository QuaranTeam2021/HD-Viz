...
export function computeCoordinates(M, N) {
	return M.map(function (row) {
		return transpose(N).map(function (column) {
			return d3.sum(d3.zip(row, column).map((i) => i[0] * i[1]));
		});
	});
}
...
export function scale(X){
	...
	var T = transpose(X);
	var m = T.map(function (row) { return d3.sum(row) / X.length; });
...
export function getEigenvalues(X, dim) {
	var USV = svd(X);
	return USV.V.map(v => v.slice(0, dim));
}

export function initProjection(X, dim) {
	return computeCoordinates(X, getEigenvalues(X, dim));
}